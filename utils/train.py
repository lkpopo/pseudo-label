...
class CrossEntropyLossOneHot(nn.Module):
    ...
    def forward(self, preds, labels):
        one_hot_labels = F.one_hot(labels, num_classes=preds.size(-1))
        return torch.mean(torch.sum(-one_hot_labels * self.log_softmax(preds), -1))
...
class System():
    def __init__(self, batch_size, model_name, pretrain, num_class, dataset_name, frac, cuda, num_epochs, EOPCH,
        ...
        self.labeled_dataset, self.unlabeled_dataset = random_split(self.dataset, [0.8, 0.2])
        ...
        os.makedirs(f'models/{self.data_name}', exist_ok=True)
        torch.save(self.model.state_dict(), f'models/{self.data_name}/{self.pretrain}_{self.model_name}ini.pth')
    ...
    def train(self, train_dataset, val_dataset, save_name, pse, fold_i):
        ...
        for epoch in range(self.num_epochs):
            ...
            with torch.no_grad():
                total_true_labels, total_predicted_labels = [], []
    ...
    def run(self):
        for fold, (train_indices, val_indices) in enumerate(kf.split(self.labeled_dataset)):
            ...
            self.model.load_state_dict(
                torch.load(f'models/{self.data_name}/{self.pretrain}_{self.model_name}ini.pth',
